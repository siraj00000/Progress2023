...
const pdpCtrl = {
    productDetais: async (req, res, next) => {
        try {
            ...
            if ("ds1/" + dsN !== label?.DS1[dsN_Index - 1]) return next(new ErrorResponse("Error 010: Missing label", 400));
            ...
            const warranty = await Warranty.where({ ds1: `ds1/${dsN}` }).findOne();

            if (productDetail) res.status(200).json({
                success: true,
                msg: "Yes exists",
                data: {
                    label, productDetail, owner, brand, warranty
                },
            });
        } catch (error) {
            next(error);
