...
const companyAdminDetail = {
    ...
    fetchSpecificCompany: async (req, res, next) => {
        try {
            const { id } = req.params;
            if (!id) return next(new ErrorResponse("Invalid identification!", 400));

            const company = await CompanyAdmin.where({ _id: id }).findOne();

            res.status(200).json({
                success: true,
                data: company.sub_category,
            });
