...
const EditCompanyAdmin = () => {
    ...
    const checkSelectedSubcategory = (id) => {
        let isMatched = true;
        for (let index = 0; index < selectedSubCategory.length; index++) {
            let subCat = selectedSubCategory[index];
            if (subCat._id === id) {
                isMatched = false;
                break;
            }
            ...
            fetchSubCategory(token, `/api/fetch-subcategory`)
                .then(res => {
                    ...
                    for (let index = 0; index < data?.data.length; index++) {
                        ...
                        let isSelectedElement = checkSelectedSubcategory(element._id);
    ...
    const handleCategorySelection = (item) => {
        let list = [...selectedSubCategory];
        list.push(item);
        setSelectedSubCategory(list);
    };

    const removeSubCategory = (index) => {
        let _subCategoryList = [...selectedSubCategory];
        _subCategoryList.splice(index, 1);

        setSelectedSubCategory(_subCategoryList);
    };
