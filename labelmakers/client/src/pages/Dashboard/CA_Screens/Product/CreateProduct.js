...
import { handleFetch } from '../../../../utils/actions/companyData';
...
const CreateProduct = ({ user }) => {
    ...
    const selectBrand = id => {
        brandList?.forEach(element => {
            if (element._id === id) {
                setBrand(element);
                setSubCategoryByCompanyId(element.company_id);
                setReqHelp(element?.request_help);
            }
        });
    };

    const setSubCategoryByCompanyId = async (company_id) => {
        try {
            const response = await handleFetch(token, `/api/fetch-spacific-company/${company_id}`);
    ...
    const selectSubCategory = index => {
        ...
        let data = subcategoryList[index];

        setCategoryId(data?.parent_category_id);
        setSubCategoryId(data._id);
        setsubCatFeatures(data.feature);
    };
