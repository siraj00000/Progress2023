...
const { sendEmail, sendVerificationEmail } = require("../lib/email-sender/sender");
const {
...
const verifyEmailAddress = async (req, res) => {
  ...
  if (isAdded) {
    ...
    const mailOption = {
      ...
      text: customerRegisterBody(option),
    };

    const message = "Please check your email to verify your account!";
    sendVerificationEmail(mailOption, res, message);
  }
};
...
const forgetPassword = async (req, res) => {
  ...
  if (!isAdded) {
    ...
    sendEmailByBrevo(body, res, message);
  }
};
