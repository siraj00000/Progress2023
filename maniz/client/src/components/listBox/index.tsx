...
export default function ListboxComponent({
  ...
  const [selected, setSelected] = useState<DropdownListType | null>(null)

  const onChange = (e: DropdownListType) => {
    setSelected(e)
    onItemSelected(e.name)
  }
  return (
        ...
        <div className="relative my-1">
          <Listbox.Button className="relative w-full cursor-pointer text-left border border-gray-200 rounded appearance-none block h-full text-gray-700 py-3 px-4 leading-tight focus:outline-none focus:bg-white">
            <span className="block truncate">
              {selected ? selected.name : `select ${placeholder}`}
            </span>
